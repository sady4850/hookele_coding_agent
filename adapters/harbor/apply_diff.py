"""Utility for applying V4A-style diffs to text.

Ported from the OpenAI Agents SDK apply_diff helper to support GPT-5.1 style
apply_patch tool semantics.
"""
...
class ConflictMarkersError(ValueError):
    """Raised when the file contains unresolved git merge conflict markers."""
    pass


def _check_conflict_markers(text: str) -> None:
    """Raise ConflictMarkersError if text contains unresolved merge conflict markers."""
    for line in text.split("\n"):
        stripped = line.strip()
        if stripped.startswith("<<<<<<< ") or stripped == "=======" or stripped.startswith(">>>>>>> "):
            raise ConflictMarkersError(
                "File contains unresolved git merge conflict markers (<<<<<<< / ======= / >>>>>>>). "
                "Resolve the conflict first by either: "
                "(1) deleting the file and creating it fresh with the correct content, or "
                "(2) using run_command to manually edit the file and remove conflict markers."
            )
...
def apply_diff(input: str, diff: str, mode: ApplyDiffMode = "default") -> str:
    ...
    # Check for conflict markers before attempting to apply diff
    _check_conflict_markers(input)
