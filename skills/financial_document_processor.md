...
**NEVER move files back to source directory to "retry" - fix extraction logic instead.**

Once files are moved to their destination directories:
- If extraction values are wrong, FIX THE EXTRACTION CODE and update the summary
- Do NOT move files back to source and reprocess - this leads to destructive loops
- Work with files in their current location
...
### 5. Moving Files Back to Retry (FATAL)
**Problem:** Moving files back to source directory to "start over" when values look wrong.
**Result:** Files end up in wrong places, summary.csv references non-existent files, all tests fail.
**Prevention:** NEVER move files back. Fix extraction code and regenerate summary.csv in place.
